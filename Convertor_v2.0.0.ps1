...
# Build the Header Art using the dynamic title ## 13 Spaces after $scriptTitle
$HeaderArt = @"
...
# Capture the script name for the Title bar
$scriptTitle = (Get-Item $PSCommandPath).BaseName
...
$global:Form.Text = "THE ARCHIVIST | $scriptTitle"
...
$global:lblGitHub.Text = "The Archivist - $scriptTitle | GitHub.com | Click for Updates"
$global:lblGitHub.Location = New-Object Drawing.Point(20, 480) # Placed neatly under the Log
$global:lblGitHub.Size = New-Object Drawing.Size(600, 20)
...
# The "Wiring" to open the browser when clicked
$global:lblGitHub.Add_LinkClicked({ 
        [System.Diagnostics.Process]::Start("https://github.com/jj1eckhardt-beep/MSG-to-DOCX-Batch-Converter") 
    })

# Assemble everything into the main form
...
# Update your assembly to include it
$global:Form.Controls.Add($global:btnRebuild)
...
function Get-HTMLHeader {
    param($ItemCount) # We'll pass the count into the header now
...
<html><head>
    ...
    <style>
        ...
        .paperclip { color: #00FF00; font-weight: bold; }
    </style>
    <script>
...
<body>
    <h1>THE ARCHIVIST | Master Index</h1>
...
function Add-IndexRow {
    ...
    $clip = if ($PdfLink) { "<a class='paperclip' href='$PdfLink'>[PDF]</a>" } else { "" }
    return @"
<tr>
    <td>$Date</td>
...
function Update-MasterIndex {
    ...
    foreach ($item in $items) {
        ...
        if ($item.PSIsContainer) {
            $hasPdf = Get-ChildItem (Join-Path $item.FullName "*.pdf") | Select-Object -First 1
            if ($hasPdf) { $pdfPath = "$($item.Name)/$($hasPdf.Name)" }
        }

        # Use the "Brick" from Function 2.2
        $content += Add-IndexRow -Date $displayDate -Subject $displaySubject -From $displaySender -MsgLink $link -PdfLink $pdfPath
        [System.Windows.Forms.Application]::DoEvents()
    }
...
function Test-PathsReady {
    if ([string]::IsNullOrWhiteSpace($global:MasterPath) -or `
            [string]::IsNullOrWhiteSpace($global:ArchivePath)) {
        [Windows.Forms.MessageBox]::Show(
            "Please select both a MASTER source and an ARCHIVE destination folder.", 
            "Missing Paths", 
    ...
    try {
        ...
        foreach ($file in $msgFiles) {
            ...
            if ($global:rbHTML.Checked) {
                ...
                $attachLinks = ""
                if ($global:chkExtract.Checked -and $msg.Attachments.Count -gt 0) {
                    foreach ($at in $msg.Attachments) {
                        if ($at.FileName -notlike "image*") {
                            ...
                            $attachLinks += "<p>Attachment: <a href='$safeAtName'>$safeAtName</a></p>"
                        }
                    }
                }
                "<html><body>$($msg.HTMLBody)<hr>$attachLinks</body></html>" | Out-File $htmlPath -Encoding utf8
